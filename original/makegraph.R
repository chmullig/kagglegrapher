...
YRANGE <- c(.4, .8)
MINDATE <- strptime("2012-10-10", "%Y-%m-%d", tz="EST")
MAXDATE <- strptime("2012-12-11", "%Y-%m-%d", tz="EST")
OUTPUT <- "datascience_leaderboard.png"
...
mindate <- max(c(min(scores$SubmissionDate), MINDATE))
maxdate <- max(c(max(scores$SubmissionDate), MAXDATE))
